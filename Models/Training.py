...
class FederatedLearning:
    def __init__(self, name: str = "", model: torch.nn.Module = None, weights: dict = None, dataTrain:torchvision.datasets = None, dataTest:torchvision.datasets = None):
        self.name = name
        ...
        for epoch in range(epochs):
            ...
            for i, data in enumerate(self.train_loader, 0):
                ...
                if mini_batches_to_print > 0 and i % mini_batches_to_print == mini_batches_to_print - 1:  
                    print(f"{self.name} Training: [{epoch + 1}, {i + 1}] loss: {running_loss / mini_batches_to_print}")
